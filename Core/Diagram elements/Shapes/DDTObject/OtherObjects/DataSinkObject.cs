...
{
    public class DataSinkObject : SmallObject
	{
		#region Fields
        ...
        {
            get { return "DataSinkObject"; }
        }
        #endregion

		#region Constructor

        public DataSinkObject()
            : base("TRANSFORM", false)
        {
            //Init(); 
        }

		public DataSinkObject(IModel s) : base(s)
		{
            //Init();
		}
        public DataSinkObject(string text)
            : base(text)
        {
            //Init();
        }

        public DataSinkObject(string text, bool shrink)
            : base(text, shrink)
        {
            //Init();
        }
		...
		public override void Paint(System.Drawing.Graphics g)
		{
            ...
            Brush DShapeBrush = Brushes.LightCyan ;
            g.FillRectangle(DShapeBrush, Rectangle); 

            //the edge of the bundle
            Pen DPen = new Pen(Color.DeepPink, 2F);

            DPen.DashStyle = System.Drawing.Drawing2D.DashStyle.Solid;
            if (Hovered || IsSelected)
            {
                DPen.Color = selectedColor;
                DPen.Width = selectedWidth;
                g.DrawRectangle(DPen, Rectangle);
            }
            else
            {
                g.DrawRectangle(DPen, Rectangle);
