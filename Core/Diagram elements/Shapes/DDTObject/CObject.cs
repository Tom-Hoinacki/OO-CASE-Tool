...
{
    public class CObject : DDTObject
	{
		#region Fields
        ...
        public CObject()
            : base("Concrete Object", false)
        {
            //Init(); 
        }

		public CObject(IModel s) : base(s)
		{
            //Init();
		}
        public CObject(string text)
            : base(text)
        {
            //Init();
        }

        public CObject(string text, bool shrink)
            : base(text, shrink)
        {
            //Init();
        }
		...
		public override void Paint(System.Drawing.Graphics g)
		{
            ...
            //edit Cobj Shape color here
            Brush CShapeBrush = new LinearGradientBrush(new Point(((int)(Rectangle.X)), ((int)(Rectangle.Y))), new Point(((int)(Rectangle.X + Rectangle.Width)), ((int)(Rectangle.Y))), Color.LightBlue, Color.WhiteSmoke);
            g.FillRectangle(CShapeBrush, Rectangle); 

            //the edge of the bundle
            Pen CPen = new Pen(Color.DarkBlue, 2.5F);

            CPen.DashStyle = System.Drawing.Drawing2D.DashStyle.Solid;
            if (Hovered || IsSelected)
            {
                CPen.Color = selectedColor;
                CPen.Width = selectedWidth;
                g.DrawRectangle(CPen, Rectangle);
            }
            else
            {
                g.DrawRectangle(CPen, Rectangle);
